...
export const createStyles: CreateStylesType<BaseTheme> = <TStyles extends StylesType>(
  stylesOrDelegate: TStyles | ((theme: BaseTheme, tools: ThemeTools) => TStyles),
  { prefix = 'react-ui' }: CreateStylesProps = {}
): UseStylesType<BaseTheme, TStyles> => {
  const makeStyles = createMakeStyles({ useTheme: () => ({}) }).makeStyles;
  const useStylesInnerFunc = makeStyles<{ theme: BaseTheme; tools: ThemeTools; }>({ name: prefix })((_ignore, { theme, tools }, classes) => {
    const raw = (is.function(stylesOrDelegate) ? stylesOrDelegate(theme, tools) : stylesOrDelegate) ?? {};
    const result = hoistMediaQueriesInStyles(raw);
