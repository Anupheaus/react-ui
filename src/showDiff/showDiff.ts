...
import { moment } from 'anux-common';
...
function formatValue(indent: number, info: Reflect.ITypeOf): string {
  if (info.isNull) {
    return 'null';
...
function diffReactElement(indent: number, infoA: Reflect.ITypeOf<React.ReactElement<any>>, infoB: Reflect.ITypeOf<React.ReactElement<any>>): string {
...
function diffNonMatching(indent: number, infoA: Reflect.ITypeOf, infoB: Reflect.ITypeOf): string {
  if (infoA.value === infoB.value) {
    return formatValue(indent, infoA);
  } else if (infoA.isDate && infoB.isDate) {
    if (infoA.value.getTime() === infoB.value.getTime()) { return formatValue(indent, infoA); }
...
function diffPrototype(infoA: Reflect.ITypeOf, infoB: Reflect.ITypeOf): string {
  if (infoA.value.name === infoB.value.name) {
...
function diffInstance(infoA: Reflect.ITypeOf, infoB: Reflect.ITypeOf): string {
  if (infoA.value === infoB.value) {
    return formatValue(0, infoA);
  } else {
    return `%rs${formatValue(0, infoA)} %gr${formatValue(0, infoB)}`;
...
function diffFunction(infoA: Reflect.ITypeOf<Function>, infoB: Reflect.ITypeOf<Function>): string {
  if (infoA.value === infoB.value) {
    return formatValue(0, infoA);
  } else {
    return `%rs${formatValue(0, infoA)} %gr${formatValue(0, infoB)}`;
...
function diffValues(indent: number, infoA: Reflect.ITypeOf, infoB: Reflect.ITypeOf): string {
