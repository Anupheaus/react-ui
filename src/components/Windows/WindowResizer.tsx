...
export const WindowResizer = createComponent({
  id: 'WindowResizer',
  ...
  render({
    ...
    const { dragInitiator: dragInitiatorTop } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingTop, onDragEnd: onDragEnd });
    const { dragInitiator: dragInitiatorTopRight } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingTopRight, onDragEnd });
    const { dragInitiator: dragInitiatorRightTop } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingTopRight, onDragEnd });
    const { dragInitiator: dragInitiatorRight } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingRight, onDragEnd });
    const { dragInitiator: dragInitiatorRightBottom } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingBottomRight, onDragEnd });
    const { dragInitiator: dragInitiatorBottomRight } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingBottomRight, onDragEnd });
    const { dragInitiator: dragInitiatorBottom } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingBottom, onDragEnd });
    const { dragInitiator: dragInitiatorBottomLeft } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingBottomLeft, onDragEnd });
    const { dragInitiator: dragInitiatorLeftBottom } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingBottomLeft, onDragEnd });
    const { dragInitiator: dragInitiatorLeft } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingLeft, onDragEnd });
    const { dragInitiator: dragInitiatorLeftTop } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingTopLeft, onDragEnd });
    const { dragInitiator: dragInitiatorTopLeft } = useDrag({ isEnabled, onDragStart, onDragging: onDraggingTopLeft, onDragEnd });

    if (!isEnabled) return null;

    return (
      <Tag name="window-resizer" className={css.windowResizer}>
        <Tag name="window-resizer-top" ref={dragInitiatorTop} className={join(css.windowResizerCommon, css.windowResizerTop)} />
        <Tag name="window-resizer-top-right" ref={dragInitiatorTopRight} className={join(css.windowResizerCommon, css.windowResizerTopRight)} />
        <Tag name="window-resizer-right-top" ref={dragInitiatorRightTop} className={join(css.windowResizerCommon, css.windowResizerRightTop)} />
        <Tag name="window-resizer-right" ref={dragInitiatorRight} className={join(css.windowResizerCommon, css.windowResizerRight)} />
        <Tag name="window-resizer-right-bottom" ref={dragInitiatorRightBottom} className={join(css.windowResizerCommon, css.windowResizerRightBottom)} />
        <Tag name="window-resizer-bottom-right" ref={dragInitiatorBottomRight} className={join(css.windowResizerCommon, css.windowResizerBottomRight)} />
        <Tag name="window-resizer-bottom" ref={dragInitiatorBottom} className={join(css.windowResizerCommon, css.windowResizerBottom)} />
        <Tag name="window-resizer-bottom-left" ref={dragInitiatorBottomLeft} className={join(css.windowResizerCommon, css.windowResizerBottomLeft)} />
        <Tag name="window-resizer-left-bottom" ref={dragInitiatorLeftBottom} className={join(css.windowResizerCommon, css.windowResizerLeftBottom)} />
        <Tag name="window-resizer-left" ref={dragInitiatorLeft} className={join(css.windowResizerCommon, css.windowResizerLeft)} />
        <Tag name="window-resizer-left-top" ref={dragInitiatorLeftTop} className={join(css.windowResizerCommon, css.windowResizerLeftTop)} />
        <Tag name="window-resizer-top-left" ref={dragInitiatorTopLeft} className={join(css.windowResizerCommon, css.windowResizerTopLeft)} />
      </Tag>
    );
  },
});
