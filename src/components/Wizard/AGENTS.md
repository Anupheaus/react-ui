...
`Actions` accepts all [`ActionsToolbar`](../ActionsToolbar/AGENTS.md) props except `isSaveReadOnly` (controlled by the wizard). `onSave` and `saveLabel` apply only on the **last step** — on earlier steps the wizard always uses Next internally. Use the remaining props to add a Delete button, confirmation dialogs, loading state, etc.
