...
| `canRemove` | `(record) => boolean` | Optional. Per-row gate for the remove action (only meaningful alongside `onRemove`). When it returns `false` for a record, that row's remove action is hidden while edit stays; when omitted, every row keeps its remove action. Use it to pin undeletable rows (e.g. a control-plane row). |
