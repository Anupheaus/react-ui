...
| `onRequest` | `ListOnRequest<T>` | No | Async callback for paginated/lazy loading. Mirrors `InternalListProps.onRequest` — see [InternalList](../InternalList/README.md) for the full contract. |
...
All `onRequest` / virtualisation behaviour is handled entirely by `InternalList`. Refer to [InternalList](../InternalList/README.md) for the virtualisation and request contract details.
